...
还会给出两条名次概率横条:**你的真实水平**(这副牌上你各名次的概率,标出实际落点)
和**你换满血 AI**(打法天花板)。所以"吃四=运气差"不再是黑箱——你能直接看到"以我的
水平这副牌大多几位",一眼判断这次是不是真背。

三项相加恒等于「实际名次距中性的偏差」,是闭合分解不是拍脑袋的权重。diagnose 跑三条
基线(均等 / 真实 / 你换满血),`--trials 20` 下 CPU 上约几分钟(已自动多进程)。
